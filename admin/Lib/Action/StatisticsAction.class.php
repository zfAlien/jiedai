...
class StatisticsAction extends CommonAction{
    ...
    public function index() {
    	if(trim($_REQUEST['search'])=="do"){
	    	...
	    	$now_time = to_timespan(to_date(get_gmtime(),"Y-m-d"),"Y-m-d");
	    	...
	    	//标分类
	    	$deal_cate = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."deal_cate where is_effect=1 and is_delete=0 order by sort desc");
	    	...
	    	for($i = 0 ; $i<=$day; $i++){
	    		$list[$i]['day'] = $day_time;
	    		//线上充值金额
	    		$list[$i]['online_pay'] = floatval($GLOBALS['db']->getOne("SELECT sum(deal_total_price) FROM ".DB_PREFIX."deal_order where pay_status=2 and type = 1 and is_delete = 0 and update_time between $day_time and $day_time+24*3600 and payment_id not in (SELECT id from ".DB_PREFIX."payment where class_name='Otherpay') "));
	    		//线下充值金额
	    		$list[$i]['below_pay'] = floatval($GLOBALS['db']->getOne("SELECT sum(deal_total_price) FROM ".DB_PREFIX."deal_order where pay_status=2 and type = 1 and is_delete = 0 and update_time between $day_time and $day_time+24*3600 and payment_id in (SELECT id from ".DB_PREFIX."payment where class_name='Otherpay') "));
